...
- ✅ Decode token with `permissions: undefined` → fallback to empty array
...
- ✅ All application permissions from `applicationScopes.json` → admin user test now imports and uses actual permissions
...
### Issue: Duplicate Test with Mismatched Name

**Issue**: The test at lines 197-217 in `react-auth0-spa.test.tsx` is named `'falls back to empty permissions array when permissions claim is null'` but actually tests the `undefined` case, not `null`. This makes it a duplicate of the previous test (lines 173-195).

**Location**: [src/auth/react-auth0-spa.test.tsx](src/auth/react-auth0-spa.test.tsx#L197-L217)
...
test('falls back to empty permissions array when permissions claim is null', async () => {
  ...
  ;(jwtDecode.default as jest.Mock).mockReturnValue({
    scope: 'openid profile email',
    aud: 'ebl-backend',
    permissions: undefined, // ❌ Tests undefined, not null
  })
  // ... rest of test ...
})
```
...
#### Fix Option 1: Test the actual `null` case (Recommended)

Replace the duplicate test to properly test `null`:
...
test('falls back to empty permissions array when permissions claim is null', async () => {
  ...
  ;(jwtDecode.default as jest.Mock).mockReturnValue({
    scope: 'openid profile email',
    aud: 'ebl-backend',
    permissions: null as any,  // ✅ Properly tests null case
  })
...
**Why this works**: The code uses `decoded.permissions || []` which handles both `undefined` and `null` the same way. By testing `null`, we verify this behavior is correct.

#### Fix Option 2: Remove duplicate and consolidate

Remove the duplicate test and update the previous test's name:

```typescript
test('falls back to empty permissions array when permissions claim is undefined or null', async () => {
  const auth0ClientMock = createDefaultAuth0ClientMock({
    isAuthenticated: jest.fn().mockResolvedValue(true),
    getUser: jest.fn().mockResolvedValue({ name: 'Test User' }),
    getTokenSilently: jest.fn().mockResolvedValue('valid-token'),
...
Then delete lines 197-217 entirely.

**Recommendation**: Use **Fix Option 1** to improve test coverage by actually testing the `null` case, which provides better verification that the fallback logic handles all edge cases.

---

## New Review Follow-up TODOs (Implemented ✅)

### TODO A: Permissions Claim Missing Behavior

- [x] Added fallback strategy in `createSession()` to use `scope` when `permissions` is missing
- [x] Defined and implemented precedence explicitly:
  - [x] `permissions` present → use `permissions`
  - [x] `permissions` missing and `scope` present → parse `scope` fallback
  - [x] both missing → use guest/basic permissions strategy
- [x] Added regression tests for scope fallback and missing-claims fallback paths

### TODO B: MemorySession Behavior Assertion (Non-smoke)

- [x] Added focused behavior tests that verify permission flip for `write:bibliography`
- [x] Verified assertion pair:
  - with permission `write:bibliography` → `isAllowedToWriteBibliography()` is `true`
  - without permission `write:bibliography` → `isAllowedToWriteBibliography()` is `false`
- [x] Implemented as deterministic session-behavior assertions (not smoke-only render checks)

### TODO C: Define Basic Permissions for Not Logged-in Users

- [x] Aligned basic/guest permission fallback with guest capabilities in `Session.ts`
- [x] Implemented baseline fallback permissions when token claims are missing:
  - [x] `read:words`
  - [x] `read:fragments`
  - [x] `read:bibliography`
  - [x] `read:texts`
- [x] Applied this baseline for authenticated sessions where both `permissions` and `scope` claims are missing
- [x] Added test coverage validating this fallback behavior
