  ...
  return (
    <Popover id={_.uniqueId('LemmatizationHelp-')} title="Lemmatization Help">
      <Popover.Content>
        ...
        <p>
          Statives should be lemmatized under the infinitive (
          <code>par-sat</code> → <code> parāsu I</code>)
        </p>
        <p>
          Most participles have independent entries (so <code>pa-ri-su</code> →{' '}
          <code>pārisu</code>). If they don’t, lemmatize them under the
          infinitive (<code>ta-bi-ku</code> → <code>tabāku</code>, since there
          is no <code>tābiku</code>).
        </p>
      </Popover.Content>
    </Popover>
  )
