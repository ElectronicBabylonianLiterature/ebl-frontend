...
const otherFragment: Fragment = produce(tokenIdFragment, (draft) => {
  draft.number = 'K.2'
})
...
beforeEach(() => {
  ...
  fragmentServiceMock.find.mockImplementation((number: string) =>
    Bluebird.resolve(
      number === tokenIdFragment.number ? tokenIdFragment : otherFragment,
    ),
  )
  fragmentServiceMock.fetchNamedEntityAnnotations.mockImplementation(
    (number: string) =>
      Bluebird.resolve(
...
describe('when the fragment changes without a remount', () => {
  it('refetches the annotations for the new fragment', async () => {
    ...
    await waitFor(() =>
      expect(
        fragmentServiceMock.fetchNamedEntityAnnotations,
      ).toHaveBeenCalledWith('K.2'),
    )
