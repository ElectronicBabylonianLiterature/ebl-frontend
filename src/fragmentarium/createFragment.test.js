...
import createFragment, { Fragment, Measures, Measure, RecordEntry, Line, Text } from './createFragment'
...
it('Creates fragment with folios', async () => {
  ...
  const expected = new Fragment({
    ...
    text: new Text({ lines: List(dto.text.lines).map(({ type, prefix, content }) => new Line({ type, prefix, content })) }),
    notes: dto.notes,
    museum: dto.museum,
