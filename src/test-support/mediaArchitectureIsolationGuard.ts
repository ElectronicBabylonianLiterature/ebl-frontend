...
export const iiifArchitectureModules = [
  'fragmentarium/domain/mediaImageService',
  'fragmentarium/domain/mediaSource',
].sort()

export const mediaDomainModule = 'fragmentarium/domain/media'

export const mediaDomainConsumers = [
  'fragmentarium/infrastructure/iiif/iiifCanvasBody',
].sort()

const mediaArchitectureRoot = 'fragmentarium/'
const mediaArchitectureFileName = /^media/i
...
export function isIiifArchitectureModule(modulePath: string): boolean {
  return iiifArchitectureModules.includes(modulePath)
}

export function isMediaArchitectureFile(relativePath: string): boolean {
  const modulePath = toModulePath(relativePath)
  return (
    modulePath.startsWith(mediaArchitectureRoot) &&
    mediaArchitectureFileName.test(path.posix.basename(modulePath)) &&
    !isIiifArchitectureModule(modulePath)
  )
}

export function isMediaDomainConsumer(relativePath: string): boolean {
  return mediaDomainConsumers.includes(toModulePath(relativePath))
}
...
export function findMediaArchitectureReferences(
  ...
  return collectModuleReferences(filePath, source)
    ...
    .filter(
      (reference) =>
        isMediaArchitectureModule(reference.specifier) &&
        !(
          isMediaDomainConsumer(filePath) &&
          reference.specifier === mediaDomainModule
        ),
    )
}
