...
/**
 * Heights are **analytical units**, deliberately not metres: nothing here
 * describes architecture, stratigraphy or ancient ground level. The ceiling is
 * fixed so no dataset can produce a tower that reads as a building.
 */
...
/**
 * Counts of findspots are small and evenly spread, so linear reads honestly.
 * Fragment counts and densities are long-tailed, so they get a log curve.
 */
...
/**
 * The reference is the 95th percentile of the *currently visible, authorized*
 * values, so the scale responds to the selected site and to what the reader may
 * actually see. It is deterministic: the same values always give the same
 * heights.
 */
...
/**
 * Unmapped polygons stay flat — they must never read as a meaningful tower.
 * A mapped polygon whose metric is zero keeps a minimal plinth so it is
 * distinguishable from one with no findspot at all.
 */
