...
test('Multiple stages', async () => {
  ...
  expectLink(
    `${Stages[multiStageText.chapters[0].stage].abbreviation} ${
      multiStageText.chapters[0].name
    }`,
    '/corpus/L/1/1/OB/The Only Chapter'
  )
  expectLink(
    `${Stages[multiStageText.chapters[1].stage].abbreviation} ${
      multiStageText.chapters[1].name
    }`,
