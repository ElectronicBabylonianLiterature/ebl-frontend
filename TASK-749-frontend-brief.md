...
_inside_ the name. The backend has split that into **two arrays**.
...
Brackets _around_ a name were never in `nameParts` and are unaffected. Only
...
with `addAccents`, so asserting here proves a name _renders_ as written rather
...
value tokens stay in `nameParts`, and the brackets that fall _inside_ a name
...
rather than a cosmetic loss. Brackets _around_ a name were never in `nameParts`
and are unaffected.
...
name is asserted to _render_ as `k ] u` rather than only to come back as a list.
