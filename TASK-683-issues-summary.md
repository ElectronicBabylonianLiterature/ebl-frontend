...
| --- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| P1  | Build/Test runtime   | Long runs exit early with `The build failed because the process exited too early.`                                                             | Root cause confirmed as OOM kill: only 2.1 GB available vs 2.5–4+ GB needed; no swap; local A/B showed `CI=true yarn build` fails while `CI=true GENERATE_SOURCEMAP=false yarn build` passes; see `TASK-683-build-investigation.md` § Root-Cause Deep-Dive | High     | Mitigation implemented  | Keep `GENERATE_SOURCEMAP=false` scoped to the CI Build step only; validate once in GitHub Actions runner context       |
...
- **Primary blocker (confirmed root cause):** OOM kill. Only ~2.1 GB available at build start vs 2.5–4+ GB needed by webpack. No swap space. SIGTERM from Codespace host agent, SIGKILL follows = exit 137. Local A/B now validates a production-safe mitigation: keep sourcemaps enabled for normal builds, but set `GENERATE_SOURCEMAP=false` only in the CI Build step.
...
1. Validate the updated GitHub Actions `test` job in runner context and confirm the CI Build step completes without the early-exit marker.
2. Validate the Docker build path in a real GitHub Actions run (`docker` or `docker-test` job), because Docker CLI is unavailable locally.
