...
- **Writes are never aborted.** `usePromiseEffect` returns `[run, cancel, runWrite]`. `run` is for reads: it hands the operation an `AbortSignal` and aborts it on unmount or supersession. `runWrite` is for saves and hands the operation an `isStale()` predicate instead of a signal, backed by `SupersedableOperation`. A superseded write keeps running to completion and only its _UI update_ is discarded — once a write has been dispatched the client cannot know whether the server applied it, so aborting the connection would lose that outcome silently. **No service or repository write method accepts a `signal`**, so a signal cannot reach a write's `fetch` through the domain layers. (`ApiClient.postJson` and `ApiClient.putJson` do still take an optional `signal`, because several reads are issued over POST — `queryByIds`, the annotation and AfoRegister queries. The guarantee lives one layer up, in the service and repository signatures.) Both `run` and `runWrite` return a promise that rejects with the operation's error, so the operation must handle its own failures (typically by rendering an `ErrorAlert`).
- **Superseding a write, outside the hook.** Components that own a single write and are not using `usePromiseEffect` hold their own `SupersedableOperation` and call `supersede()` from `componentWillUnmount` (or a `useEffect` cleanup): `TransliterationForm`, `WordEditor`, `BibliographyEntryFormController` and `BibliographyEntryForm`. `usePromiseEffect`'s own cleanup deliberately does **not** supersede the write it is holding — `cancel()` and unmount abort the _read_ only, so a save started from a component that then unmounts still runs its `onSuccess`; under React 18 the resulting `setState` is a silent no-op. `AbortableOperation` is now a read-only primitive: it is used by `usePromiseEffect`'s read slot and by `CuneiformConverterForm`, and by nothing that writes.
