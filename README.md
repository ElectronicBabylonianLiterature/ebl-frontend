...
- **Writes are never aborted.** `usePromiseEffect` returns `[run, cancel, runWrite]`. `run` is for reads: it hands the operation an `AbortSignal` and aborts it on unmount or supersession. `runWrite` is for saves and hands the operation an `isStale()` predicate instead of a signal, backed by `SupersedableOperation`. A superseded write keeps running to completion and only its _UI update_ is discarded — once a write has been dispatched the client cannot know whether the server applied it, so aborting the connection would lose that outcome silently. **`ApiClient.postJson` and `ApiClient.putJson` do not accept a `signal` at all**, and `ApiClient.fetch` — the only method that does take one alongside an arbitrary `method` — is `private`. A signal therefore cannot reach a write's `fetch` from outside the class, so the guarantee is enforced by the type system rather than by convention. Reads that are issued over POST (`queryByIds`, the annotation and AfoRegister queries) are guard-based for the same reason as the shared-cache paths below. Both `run` and `runWrite` return a promise that rejects with the operation's error, so the operation must handle its own failures (typically by rendering an `ErrorAlert`).
- **Superseding an operation that cannot take a signal, outside the hook.** Components that own such an operation and are not using `usePromiseEffect` hold their own `SupersedableOperation` and call `supersede()` from `componentWillUnmount` (or a `useEffect` cleanup): `TransliterationForm`, `WordEditor`, `BibliographyEntryFormController` and `CuneiformFragment` own a **write**, and `BibliographyEntryForm` owns a **read** — its `loadOperation` guards `Cite.async`, which is citation-js and accepts no signal, so supersession is the only guard available to it. `CuneiformFragment` supersedes on unmount and again whenever the displayed fragment changes, so a save dispatched for the previous fragment can no longer update the new one. Where the write is debounced, the timer has to be cancelled too — `BibliographyEntryForm` calls `doLoad.cancel()` alongside `supersede()`, because a debounce that fires after unmount would otherwise call `start()` again and mint a fresh, non-stale token. `usePromiseEffect`'s cleanup aborts the read and supersedes the write it holds without aborting the write's request, so a save started from a component that then unmounts still reaches the server but its handlers no longer run. `cancel()` aborts the read only. `AbortableOperation` is now a read-only primitive: it is used by `usePromiseEffect`'s read slot and by `CuneiformConverterForm`, and by nothing that writes.
- **Writes that cannot be aborted must not overlap.** Superseding only hides an older write's result; the backend still applies it, possibly after the newer one. `CuneiformFragment` therefore routes every fragment save through a `SerialQueue`, so saves reach the server in the order they were made, and the date editors disable Save, Delete and Add while a date save is pending.
- **Applying a result only when it is still current.** `applyWhenCurrent(operation, { onSuccess, onError })(isStale)` is the one place the "run it, then apply the result unless it has been superseded" guard lives; `applyWhenNotAborted(operation, signal, handlers)` is the same guard keyed on `signal.aborted` instead. Use one of them rather than hand-writing `if (!isStale())` / `if (!signal.aborted)` around each handler.
- **Shared cached requests must not take a signal.** `getOrFetchCachedValue` hands the _same_ in-flight promise to every caller for a given key, so a per-caller abort would reject the request for all the other live consumers. The `fetchValue` callback therefore takes no arguments, and the cached paths in `FragmentService` (`find`, `fetchProvenances`, `fetchProvenance`, `fetchProvenanceChildren`, `query`, `queryLatest`, `findThumbnail`), `TextService`, `DossiersService.queryByIds` and `BibliographyService` stay guard-based on purpose.
- **Reads that take no signal.** Besides the shared-cache paths above, a read omits `signal` only when no caller owns a controller to pass — every one of these is an async-select loader or a script, not a getter: `fetchColophonNames`, `searchBibliography`, `findLemmas`, `findSuggestions`, `searchLemma` and `collectLemmaSuggestions` (react-select `loadOptions` and the lemmatisation editors), and `listAllFragments` (sitemap generation). Everything else reachable from a `withData` getter or `run` threads one **all the way into `fetch`**. A `signal` parameter that is accepted and then dropped is a defect, not a harmless no-op, and TypeScript will not catch it: an implementation declaring _fewer_ parameters is assignable to a port that declares a `signal`, so `FragmentRepository.random()` satisfied `FragmentInfoRepository.random(signal?)` while silently discarding it. `FragmentRepository.abortSignal.test.ts` asserts the forwarding for every fragment read that claims to be abortable, and `withDataGetters.abortSignal.test.tsx` asserts that getters hand their signal on; extend them when you add another. A repository that falls back to a default on failure must rethrow an `AbortError` first, or a cancellation turns into a successful empty result.
...
- **`isCancellation` is deliberately wider than `isAbortError`.** `isAbortError(error)` is true only for a genuine `AbortError`; `isCancellation(error, signal)` is _also_ true whenever `signal.aborted` is set, whatever the error is. That second clause is what suppresses the UI update for a getter that does not thread the signal and therefore rejects with something other than an `AbortError` after its view has gone away. The cost is that a real bug thrown from inside a getter at the exact moment of unmount is not surfaced to the user. It is not lost, though: `ApiClient` rethrows `AbortError` untouched and reports every other failure to Sentry before it ever reaches this check, so only a failure originating _after_ the HTTP layer - in a domain factory, say - goes unreported. Prefer `isAbortError` where you only mean "was this aborted", and keep `isCancellation` for the "can this result still be applied" question at the React boundary.
