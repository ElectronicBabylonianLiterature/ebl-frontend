...
- **Migration script**: Write a migration script to add `isReconstructed` and `isEmended` fields to all date objects in the database, defaulting to `null` (or omitting if not present).
...
- Update all endpoints that accept or return date fields (e.g., fragment create/update, corpus text endpoints) to:
  - Accept `isReconstructed` and `isEmended` in incoming payloads.
...
- Update date parsing logic to:
  ...
  - Prefer structured metadata (`isReconstructed`, `isEmended`) over inferring from raw symbols.
  ...
  - Convert `n?`, `n!`, `(n)`, `[n]`, `<n>` to use structured metadata instead.
    ...
    - Preserve the original value for display if needed, but prefer the structured metadata for all logic and conversion.
